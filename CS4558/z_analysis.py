...
directory = os.fsencode('~/OneDrive - Naval Postgraduate School/5 Quarter/NetTraff/CZA')

for filename in os.listdir(directory):
    if filename.startswith("2018") or filename.endswith(".txt"):
        print ('yes')


with open('20181126_C_outFile.txt') as csv_file:
...
day_dict = dict_of_sites
for url in day_dict:
    index_into_nested_list = 0
    for hourly_bin in day_dict[url]:
        ...
        # hour in the day
        day_dict[url][index_into_nested_list] = round(hourly_bin, 3)
...
# put the day dict items into a date dictionary
date = {}
