...
int main(int argc, char *argv[])
{
        ...
        // most of this comes from "man getaddrinfo"
...
//        hints.ai_protocol = 0;
        ...
        e = getaddrinfo (argv[DEST_ARG], "DEST_PORT", &hints, &start);
        if ((e != 0) || (start == NULL)) {
                fprintf(stderr,"DNS failure for %s: %s\n",argv[DEST_ARG], gai_strerror (e));
                exit(-1);
        }
        ...
        e = connect(Sock_fd, current-> ai_addr, current-> ai_addrlen/*sizeof(addr)*/);
        if(e != 0) {
        ...
        */
        ...
        strcat(request_buf,"USER-Agent:mywget/0.1 (linux-gnu)\r\n");
        strcat(request_buf,"Accept: text/html\r\n");
        strcat(request_buf,"Host: argv[DEST_ARG]\r\n");
        ...
        // TBD student code here...
        num = write(Sock_fd, request_buf, strlen(request_buf));
        if ((num == 0) || (errno != 0)) {
