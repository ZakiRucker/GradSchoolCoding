...
int main(int argc, char *argv[])
{
        ...
        // most of this comes from "man getaddrinfo" and connect1.c
        bzero(&hints, sizeof(hints));
        hints.ai_family = AF_INET;
        hints.ai_socktype = SOCK_STREAM;

        result = getaddrinfo (argv[DEST_ARG], DEST_PORT, &hints, &start);
        ...
        for (current = start; current != NULL; current = current->ai_next) {
                ...
                if (connect (Sock_fd, current-> ai_addr, current-> ai_addrlen) != -1) {
                        break; // connected!!
                }
                close(Sock_fd);
        }
