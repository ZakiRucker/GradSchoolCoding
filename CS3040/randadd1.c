...
// 2017-10-21 (Z. Rucker)
//     corrected iterative free 
//     corrected invalid read 
//     corrected mis-spelled pointer 
//     completed print_list function
//              created an i to count 
//              built while loop 
//              print statement 
//              increment to next list node
//              increment i 
//     completed calc_total function
//              built while loop 
//              add value to total 
//              increment to next list node 
//
...
int print_list(value_t *start)
{
        ...
        if (start == NULL) {
                ...
                while(start != NULL){
                        printf ("Number %*d = %*ld\n",PRINT_JUSTIFIED, i,PRINT_JUSTIFIED, start->val);
                        start = start->next;
                        i++;
                }
        }
...
int main(void)
{
        ...
        int result = SUCCESS;
        value_t *start = NULL;
        value_t *tmp = NULL;//pointer spelling corrected
        long int total = 0;
        ...
        if (result == SUCCESS) {
                ...
                while (start != NULL) {
                        tmp = start;
                        start = tmp->next;//**valgrind error *corrected
                        free(tmp);
                }
                start = NULL;
                tmp = NULL;
